...
.get((req,res)=>{
    ...
    query('select * from wbiao').then(function(data){
...
.post((req,res)=>{
    ...
    let id = Date.now();
    let {name,price,img,num} = req.body;
    ...
    let sql = `insert into cart values('${name}','${price}','${img}',${num})`;
    console.log(sql);
...
.get((req,res)=>{
    ...
    query(`select * from wbiao where id=${id}`).then(function(data){
        // data: promise对象改成resolve状态时传回的参数
        res.send(formatData({data}))
    },(err)=>{
        res.send(formatData({data:err,code:250}))
